...
void
relaxation::matrixUpdate(inputMatrix* newMatrix)
{
...
void
relaxation::paintEvent(QPaintEvent* event)
{
  ui->list->clear();
  for (int i = 0; i < x.size(); ++i) {
    QString str =
      "X" + QString::number(i + 1) + ":" + QString::number(x[i].back());
    ui->list->addItem(str);
  }
}

void
relaxation::on_epsSpin_valueChanged(double arg1)
{
...
void
relaxation::on_wSpin_valueChanged(double arg1)
{
  if (arg1 > 1 || arg1 <= 0) {
    return;
  }
...
void
relaxation::on_list_currentRowChanged(int currentRow)
{
  if (currentRow < 0) {
    return;
  }
  curRow = currentRow;
  ui->chart->setData(x[curRow]);
}
